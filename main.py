...
from kb import KB  # Import the KB class from kb.py
import wikipedia
import os
...
def get_news_links(query, lang="en", region="US", pages=1, max_links=10000):
    ...
    for page in range(pages):
        ...
        # Remove query parameters and prefix from URLs
        clean_urls = [url.split('url=')[1].split('&ved=')[0] for url in all_urls]
    return list(set(clean_urls))[:max_links]
...
def from_urls_to_kb(urls, verbose=False):
    ...
    for url in urls:
        ...
        try:
            ...
            if verbose:
                print(f" Couldn't download at URL: {url} Error: {e}")
    return kb
...
news_links = get_news_links("UX design interview", pages=2, max_links=20)
kb = from_urls_to_kb(news_links, verbose=True)
filename = "UX_Network_821.html"
